...
class generator:
    ...
    def __call__(self, inputs, reuse=False):
        with tf.variable_scope(name_or_scope=self.name, reuse=reuse):
            ...
            with tf.variable_scope("c7s1-64"):
                inputs = tf.nn.relu(InstanceNorm(conv(inputs, 64, 7, 1, "VALID")))
            with tf.variable_scope("d128"):
                inputs = tf.nn.relu(InstanceNorm(conv(inputs, 128, 3, 2, "SAME")))
            with tf.variable_scope("d256"):
                inputs = tf.nn.relu(InstanceNorm(conv(inputs, 256, 3, 2, "SAME")))
            for i in range(9):
                with tf.variable_scope("R"+str(i)):
                    temp = inputs
                    with tf.variable_scope("R_conv1"):
                        inputs = tf.pad(inputs, tf.constant([[0, 0], [1, 1], [1, 1], [0, 0]]), "REFLECT")
                        inputs = tf.nn.relu(InstanceNorm(conv(inputs, 256, 3, 1, "VALID")))
                    with tf.variable_scope("R_conv2"):
                        inputs = tf.pad(inputs, tf.constant([[0, 0], [1, 1], [1, 1], [0, 0]]), "REFLECT")
                        inputs = InstanceNorm(conv(inputs, 256, 3, 1, "VALID"))
                    inputs = temp + inputs
            with tf.variable_scope("u128"):
                inputs = tf.nn.relu(InstanceNorm(deconv(inputs, 128, 3, 2)))
